...
<img src="./.readme/screen/01-overview.png" width="880" alt="The matrix condenses host and service health into a color-coded overview; selecting a cell opens the exact problem context.">
...
<table>
<tr>
<td width="50%" align="center" valign="top"><img src="./.readme/screen/02-matrix.png" alt="Matrix"><br><small><b>Matrix</b> - matrix</small></td>
</tr>
</table>
...
They are one purchase and one version, not two products - a grid you cannot drill into is half a feature. The **detail widget needs Zabbix 7.0** because that is the release where Zabbix gained widget-to-widget communication; on 6.0, 6.2 and 6.4 a dashboard has no way to wire one widget's selection into another's, so the package does not install the tile there. **The detail itself is not lost on those versions:** clicking a cell opens the same host, inventory, triggers, event details and actions as a dialog on top of the grid, with the same Update (acknowledge), Execute now, History and Graph controls. Same capability, one package - the frontend decides whether it lives in a second tile or in a popup.

<div align="center">
<img src="./.readme/screen/03-cell-detail-popup.png" width="620" alt="The cell detail as a dialog on Zabbix 6.4">
<br><small><b>The cell detail on Zabbix 6.0 - 6.4</b> - the same host card, trigger card, event details and actions, opened by a click on the cell.</small>
</div>
...
| Cell detail - the problem, event, inventory and actions behind a cell: a companion widget on 7.0+, a popup on 6.0 - 6.4 | ✅ |
...
| **Widgets** | matrixMAX (6.0 - 7.4) · matrixMAX-detail (7.0 - 7.4; on 6.0 - 6.4 the cell detail opens as a popup) - both from one package |
...
The grid, the colours, the context menus, both "Load ..." buttons, the row ordering and the whole configuration dialog - same fields, same labels, same order - are identical on 6.0, 6.2, 6.4, 7.0, 7.2 and 7.4. One thing takes a different shape below 7.0, because those frontends cannot provide the mechanism behind it:

- **The cell detail.** Zabbix gained widget-to-widget communication in 7.0; before that a dashboard has no way to wire one widget's selection into another's filter. So on 6.0, 6.2 and 6.4 the package installs the grid only, and a click on a cell opens the detail as a dialog instead of feeding a second tile - the same host card, inventory, trigger cards, event details, actions and buttons. Everything the cell itself offers - its colour, its hint and its right-click menu into problems, history, the trigger and the items - works exactly as on 7.x.
