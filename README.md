...
<img src="./.readme/badge/version.svg" alt="version 2.6.1">
...
Switch **Rows by** between **Hosts**, **Host groups** and **Services**. In Services mode, rows come directly from Zabbix Services and columns match the services' own tags, not trigger tags. Tag autocomplete and loading use only accessible service tags. Each matching cell shows the service's computed status, including a green OK state. Host filters and problem-severity filters do not apply to this mode.

Enable **Show problem preview on hover** to see the accessible problems behind a cell without leaving the matrix: event time, severity, shortened operational data and links to the full event, with **Update** available when your permissions allow it. The preview is off by default. Service details show the service status, description, tags and accessible related problems; operational data belongs to those problems, not to the service itself.

One familiar widget form. Choose the **host groups** (and the ones to exclude), narrow further by host or host tag, then press **Load tags "matrix"** to fill the columns from the trigger tags your hosts actually carry. To reuse another accessible trigger tag, search globally by a full or partial **tag name** or **tag value**; these inputs use the same permission-aware vocabulary as their autocomplete, so every offered suggestion can be loaded. A load with no matches says so and leaves every existing row untouched. **Load hosts** fills the rows. All loaders **append**: rows you typed or dragged keep their place and are never overwritten, and any tag name works as a column source - write your own tag and value straight into a row. Decide with **Rows by** whether a row is one host or a whole **host group** - a group cell shows the worst problem found on any of the group's hosts, and a host that belongs to two shown groups counts in both rows. Pick which **severities** count, decide whether **suppressed problems** show, and order the rows by host id, host name, severity - or drag them into your own order.

<div align="center">
<img src="./.readme/screen/06-settings.png" width="440" alt="matrixMAX configuration">
</div>

The detail tile has a form of its own: **Inventory fields** builds the host card line by line. Give each line your own label and a value - usually a host inventory macro such as `{INVENTORY.OS}`, or an `{INVENTORYMAX.*}` one when [inventoryMAX](https://www.initmax.com/product/inventorymax/) is installed (the form tells you whether it found it). Tick **Show** to lift a line - up to three - next to the host name; the heading's tooltip explains this option. The rest live in the fold-away Inventory drawer. Drag the rows to order them. Fields, checkboxes and drag handles stay compact and aligned, with values wrapping onto a second line in narrow dialogs. Leave the list empty and the card shows the host's own filled inventory instead. Same list, same behaviour, on 6.0 through 7.4.

Turn off **Show inventory** to hide inventory from both the host header and the expandable drawer. Saved custom fields are kept for when you turn it back on; while hidden, inventory data and inventory macros are not loaded. The option defaults to on, preserving existing host cards after an upgrade.

The detail lists active problems before OK checks, using the current problem severity rather than the trigger's configured severity. Problems shown in the matrix preview remain available in the detail even if the trigger's tags have changed since the event was created. Refresh and Close stay vertically stacked and visible beside the host card.

Enable **Receive from all matrixMAX widgets** to follow whichever matrix you click on the current dashboard page, without selecting each source separately. On 7.0+ you can turn this off and keep explicitly selected sources; older frontends always follow all matrices on the page. **Show item configuration** and **Show trigger configuration** independently control the configuration buttons, without bypassing Zabbix permissions.

**Custom scripts** combines visibility and source in one selector: **None**, **Host**, **Problem** or **All**. New detail widgets default to None. Other modes open Zabbix's native script menus on demand; Zabbix filters available scripts for the selected target and your permissions, and retains its confirmation and manual-input dialogs. Problem scripts require an event, so an OK check does not offer them. Existing script visibility and source settings are preserved when upgrading.

<div align="center">
<img src="./.readme/screen/detail-settings.png" width="620" alt="Detail configuration: all matrix sources, native script selection, configuration buttons and compact inventory fields">
</div>

In the main matrix, enable **Send only problems to detail** to keep OK cells visible without loading or sending their details. Mixed cells send only their active problems, using the matrix's existing problem data rather than an extra lookup. The option is off by default. Detail severity panels keep consistent sizing for both OK and problem states while long text wraps inside the widget.
...
| **matrixMAX-detail** | Put it beside the grid: click any cell and it shows that problem's severity, age, operational data, event details and the host inventory lines you chose | 6.0 - 7.4 |

They are one purchase and one version, not two products - a grid you cannot drill into is half a feature. On 7.0+ the dashboard wires the pair together with Zabbix's own widget-to-widget communication. **6.0 - 6.4 predate that mechanism, so the package bridges it itself:** the same detail tile is there and the grid feeds it directly. You place and configure the pair exactly as on 7.4, and a dashboard built on 6.2 survives an in-place upgrade to 7.0+ with nothing to reconfigure - the native wiring simply takes over. Wherever no companion tile is placed on the dashboard, clicking a cell opens the same host, inventory, triggers, event details and actions as a dialog on top of the grid, with the same Update (acknowledge), Execute now, History and Graph controls. Same capability, one package - the frontend decides whether it lives in a second tile or in a popup.

<div align="center">
<img src="./.readme/screen/cell-detail-popup-6x.png" width="620" alt="The cell detail as a dialog">
<br><small><b>The cell detail as a dialog</b> - on any version when no companion tile is placed: the same host card, trigger card, event details and actions, opened by a click on the cell.</small>
</div>
...
| Trigger-tag autocomplete plus combined name/value loading, with non-destructive no-result feedback | ✅ |
...
| Native Zabbix Services rows, service-tag autocomplete and colour-coded service status | ✅ |
| Optional problem preview on hover with operational data, event links and Update actions | ✅ |
| Suppressed problems hidden or shown | ✅ |
| Cell detail - the problem, event, inventory and actions behind a cell: the matrixMAX-detail companion tile, or a popup wherever no companion is placed | ✅ |
| Follow all matrices on the dashboard page or choose individual sources on Zabbix 7.0+ | ✅ |
| Optional problems-only detail delivery while OK cells remain visible | ✅ |
| Independent item and trigger configuration buttons with permission checks | ✅ |
| On-demand native custom scripts: None, Host, Problem or All | ✅ |
| Inventory fields of the detail's host card - your own labels, {INVENTORY.*} and {INVENTORYMAX.*} macros, up to three lifted into the card header, drag to reorder | ✅ |
...
| **Widgets** | matrixMAX (6.0 - 7.4) · matrixMAX-detail (6.0 - 7.4; with no companion placed, the cell detail opens as a dialog) - both from one package |
...
The grid, the colours, the context menus, both "Load ..." buttons, the row ordering, the host-group rows, the detail's **Inventory fields** list and the whole configuration dialog - same fields, same labels, same order - are identical on 6.0, 6.2, 6.4, 7.0, 7.2 and 7.4. One control takes a different shape below 7.0, because that frontend cannot provide the mechanism behind it:

- **Choosing which matrix the detail follows.** On 7.0+ a widget can name another widget, so the detail's form offers a **matrixMAX widget** picker. Zabbix 6.0 - 6.4 have no such notion, so the picker is not there and the detail simply follows every matrix on the dashboard - which is the same thing on the dashboards this pair is built for. Everything else about the tile, its Inventory fields and the rendered detail is identical.
